...
export default function NonMandatoryStudentReports() {
  const [selectedSemester, setSelectedSemester] = useState<string>("");
  const { toast } = useToast();

  const { data: reportData, isLoading } = useQuery({
    queryKey: ['/api/reports/non-mandatory-students', selectedSemester],
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(14);
      doc.text("RELATÓRIO DE ESTUDANTES EM ESTÁGIOS NÃO OBRIGATÓRIOS", pageWidth / 2, 70, { align: "center" });
      ...
      if (reportData && reportData.length > 0) {
        const tableData = reportData.map((student: any) => [
          ...
          student.company || 'N/A',
          student.advisor || 'N/A',
          student.workload || 'N/A'
        ]);
        
        autoTable(doc, {
          head: [['Nome do Estudante', 'Matrícula', 'Curso', 'Empresa', 'Orientador', 'Carga Horária']],
          body: tableData,
          startY: yPosition,
          margin: { left: 10, right: 10 },
          styles: { fontSize: 8 },
          headStyles: { fillColor: [255, 140, 0], fontSize: 8 },
          columnStyles: {
            0: { cellWidth: 35 }, // Nome
            ...
            5: { cellWidth: 25 }  // Carga Horária
          },
          theme: 'striped'
        });
      } else {
        doc.setFont("helvetica", "italic");
        doc.text("Nenhum estudante em estágio não obrigatório encontrado para este período.", 20, yPosition);
      }
      
      const fileName = `estudantes_estagio_nao_obrigatorio_${selectedSemester.replace('-', '_')}semestre.pdf`;
      doc.save(fileName);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Estudantes em Estágios Não Obrigatórios</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Selecione o semestre para gerar o relatório de estudantes em estágios não obrigatórios
          </CardDescription>
        </CardHeader>
